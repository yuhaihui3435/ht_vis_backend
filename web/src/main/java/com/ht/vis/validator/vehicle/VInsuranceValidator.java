...
public class VInsuranceValidator extends CoreValidator {
    ...
    protected void validate(Controller controller) {
        ...
        if (ak.contains("vInsurance/save")) {
            ...
            list=vInsuranceService.findByLicensePlateInDate(vInsurance);
            if (!list.isEmpty()) {
                addError(Consts.REQ_JSON_CODE.fail.name(), "保险日期重叠");
                return;
            }
            ...
            list=vInsuranceService.findByLicensePlateInDateNeId(vInsurance);
            if (!list.isEmpty()) {
                addError(Consts.REQ_JSON_CODE.fail.name(), "保险日期重叠");
                return;
            }
